...
program.command('build:django')
    ...
    .action(async (options) => {
      selectfolder = (process.env.npm_config_folder != undefined)
      ...
      versions.forEach(version => {
        if (selectfolder == undefined || selectfolder == version) {
          cmd.push('docker build -t koromerzhin/django:' + version +
                   ' images/django/' + version + ' --target build-django-' +
                   version);
          if (versions[versions.length - 1] == version) {
            cmd.push('docker image tag koromerzhin/django:' + version +
...
program.command('build:php:fpm')
    ...
    .action(async (options) => {
      ...
      versions.forEach(version => {
        if (selectfolder == undefined || selectfolder == version) {
          cmd.push('docker build -t koromerzhin/php:' + version +
                   '-fpm images/phpfpm/' + version + ' --target build-phpfpm-' +
                   version);
          cmd.push('docker build -t koromerzhin/php:' + version +
                   '-fpm-xdebug images/phpfpm/' + version +
                   ' --target build-phpfpm-xdebug-' + version);
          if (versions[versions.length - 1] == version) {
            cmd.push('docker image tag koromerzhin/php:' + version +
...
program.command('build:php:apache')
    ...
    .action(async (options) => {
      ...
      versions.forEach(version => {
        if (selectfolder == undefined || selectfolder == version) {
          cmd.push('docker build -t koromerzhin/php:' + version +
                   '-apache images/php-apachep/' + version +
                   ' --target build-php-apache-' + version);
          cmd.push('docker build -t koromerzhin/php:' + version +
                   '-apache-xdebug images/php-apache/' + version +
                   ' --target build-php-apache-xdebug-' + version);
          if (versions[versions.length - 1] == version) {
            cmd.push('docker image tag koromerzhin/php:' + version +
